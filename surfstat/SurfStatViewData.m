function [ a, cb ] = SurfStatViewData( data, surf, title, background,map);
...
clf;
colormap(map)
% map = colormap(spectral(256));
% cl= zeros(size(data(vl)));
% map = colormap(jet(256));
% map(101,:)=ones(1,3);
% dl = data(vl);
% for  i =1:253  
% cl((dl>-10+0.1*(i-1))&(dl<=-10+0.1*i))=i+1;
% end
% cl(dl<=-10)=1;
% cl(dl>15.3)=256;
h=0.39;
...
trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),double(data(vl)),'EdgeColor','none');
% trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),cl,'EdgeColor','none');
view(-90,0); 
...
% f=getframe(gcf);
% imwrite(f.cdata,['/share/home1/zhaoqi/Downloads/ABCD/ABCD_release2/','test3.tiff'],'Resolution',600);
